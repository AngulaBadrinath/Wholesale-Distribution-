...
### 1.2.2 Resource Scope Enforcement (`RBAC` / `FEAT-RBAC-003` & `QA-002`)
- [x] **Centralized Resource Scope Service:** `ResourceScopeService` provides centralized domain access predicates (`canAccessCustomer`, `canAccessOrder`, `canAccessDelivery`, `canAccessReturn`, `canAccessPayment`, `canAccessInvoice`, `canAccessInventoryBalance`, `canAccessAdjustment`), query scopers (`scopeCustomers`, `scopeOrders`, `scopeDeliveries`, `scopeReturns`, `scopePayments`, `scopeInvoices`, `scopeInventoryBalances`), and parent-child ownership verification.
- [x] **Model Policy Coverage Across All Domains:** Comprehensive Laravel Policies registered and enforcing resource scopes: `CustomerPolicy`, `OrderPolicy`, `OrderAdjustmentPolicy`, `DeliveryPolicy`, `ReturnRequestPolicy`, `PaymentPolicy`, `InvoicePolicy`, `InventoryBalancePolicy`, `StockExceptionPolicy`.
- [x] **Gate::before Policy Delegation:** `AppServiceProvider` updated so `Gate::before` returns `null` when checking canonical abilities with model arguments, allowing registered Model Policies to evaluate instance ownership and scope without being bypassed by broad role permissions.
- [x] **Fail-Closed Anti-IDOR (404 Not Found):** Cross-salesman and cross-driver unauthorized resource lookup returns 404 Not Found rather than 403 Forbidden to prevent existence enumeration (`RULE-SEC-003`).
- [x] **Category A — Legitimate Scoped Access:** Salesman can view own assigned customer, order, invoice, payment, and return; Driver can view assigned delivery; Warehouse Manager can view inventory balances; Admin has broad operational access (`AuthorizationScopeTest::test_salesman_can_view_own_assigned_customer`..`test_admin_and_super_admin_have_broad_operational_access`).
- [x] **Category B — Cross-Salesman IDOR:** Salesman A strictly denied from accessing Salesman B's customer (403/404), order (404), invoice (404), return (404), or creating order/adjustment for Salesman B's customer (`test_salesman_a_cannot_view_salesman_b_customer_fail_closed`..`test_salesman_a_cannot_request_adjustment_on_salesman_b_order_fail_closed_404`).
- [x] **Category C — Cross-Driver IDOR:** Driver A strictly blocked from viewing Driver B's delivery manifest (404), pickup, out-for-delivery, complete, or fail Driver B's delivery (`test_driver_a_cannot_view_driver_b_delivery_fail_closed_404`..`test_driver_a_cannot_fail_driver_b_delivery_404`).
- [x] **Category D — Warehouse Inventory Boundaries:** Salesman, Driver, and Accountant denied from manual stock adjustment or exception reporting; Warehouse Manager authorized (`test_salesman_cannot_post_inventory_adjustment_403`..`test_warehouse_manager_can_post_inventory_adjustment`).
- [x] **Category E — Nested Resource IDOR & Parent-Child Integrity:** Mismatched parent-child identifiers (e.g. Order A ID with Adjustment B ID) fail-closed with 404 Not Found (`test_cannot_withdraw_adjustment_under_mismatched_foreign_order_fail_closed_404`, `test_cannot_review_adjustment_under_mismatched_foreign_order_fail_closed_404`).
- [x] **Category F — Financial Ledger & Payment Security:** Salesman and Driver cannot verify payments or access GL journals; Accountant authorized (`test_salesman_cannot_verify_payment_403`..`test_accountant_can_view_general_ledger`).
- [x] **Category G — Segregation of Duties / Maker-Checker:** Requester cannot approve own order, order adjustment, payment, or return request unless Super Admin emergency resolution (`test_salesman_cannot_approve_order_403`..`test_super_admin_can_resolve_emergency_maker_checker_approvals`).
- [x] **Category H — Route Parameter Tampering & Existence Disclosure:** Sequential URL tampering and unassigned foreign lookups return identical 404 status as non-existent IDs, preventing existence enumeration (`test_direct_route_url_tampering_with_foreign_ids_fails_closed`, `test_unassigned_foreign_order_lookup_returns_404_same_as_nonexistent_id`, `test_unassigned_foreign_delivery_lookup_returns_404_same_as_nonexistent_id`).
- [x] **Category I — Cross-Role Workspace Access:** Salesman denied delivery workspace, Driver denied salesman workspace, Accountant denied physical stock/dispatch mutations (`test_salesman_cannot_access_delivery_driver_workspace`..`test_accountant_cannot_modify_delivery_or_physical_stock`).
- [x] **Category J — Inactive Account Blanket Deny:** Suspended, invited, or disabled accounts have zero access to all operational endpoints (`test_suspended_user_cannot_access_any_domain_endpoints`).
- [x] **Category K & L — Authoritative Server Boundary & Zero Client Trust:** Client-supplied salesman_id, driver_id, or approved_by payloads ignored; server resolves authoritative identity (`test_client_supplied_salesman_id_override_is_ignored_on_order_creation`, `test_client_supplied_driver_id_override_is_ignored_on_delivery_update`).
