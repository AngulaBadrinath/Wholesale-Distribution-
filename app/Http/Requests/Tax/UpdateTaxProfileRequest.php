...
class UpdateTaxProfileRequest extends FormRequest
{
    ...
    {
        /** @var TaxProfile|int|string|null $taxProfile */
        $taxProfile = $this->route('tax_profile') ?? $this->route('taxProfile');
        $profileId = $taxProfile instanceof TaxProfile ? $taxProfile->id : $taxProfile;

        return [
            'name' => ['required', 'string', 'max:100'],
            'code' => ['required', 'string', 'max:50', Rule::unique('tax_profiles', 'code')->ignore($profileId)],
    ...
    {
        return [
            'code.unique' => 'This tax profile code is already in use by another profile.',
