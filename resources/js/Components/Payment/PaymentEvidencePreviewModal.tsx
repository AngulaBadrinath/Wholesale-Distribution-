...
export function PaymentEvidencePreviewModal({
    ...
    return (
        ...
        >
            <div className="relative w-full max-w-4xl bg-card rounded-xl shadow-2xl border border-border flex flex-col max-h-[90vh] overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/40">
                    <div>
                        <div className="flex items-center gap-2">
                            <h2 id="evidence-modal-title" className="text-base sm:text-lg font-bold text-foreground">
                ...
                {evidenceData && !loading && (
                    <div className="flex flex-wrap items-center justify-between px-6 py-2 bg-muted/30 border-b border-border text-xs gap-2">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-mono text-muted-foreground min-w-[40px] text-center text-xs">
                ...
                {/* Viewport */}
                <div className="relative flex-1 min-h-[360px] max-h-[60vh] overflow-auto p-4 bg-slate-950 flex items-center justify-center">
                    {loading && (
                    ...
                    {error && !loading && (
                        <div className="flex flex-col items-center gap-3 p-6 text-center max-w-md bg-destructive/10 rounded-lg border border-destructive/30 text-destructive">
                    ...
                    {evidenceData && !loading && (
                        ...
                        >
                            <img
                                ...
                                className="max-w-full max-h-[55vh] object-contain rounded shadow-lg border border-slate-800 select-none"
                            />
                        </div>
                    )}
                </div>
                ...
                {/* Footer */}
                <div className="flex items-center justify-between px-6 py-3 border-t border-border bg-muted/40 text-xs text-muted-foreground">
                    <div className="flex items-center gap-2">
                        <FileImage className="h-4 w-4 text-muted-foreground" />
                        <span className="font-mono">{payment.evidence_original_name || 'cheque_scan.jpg'}</span>
                    </div>
                    <Button
                        type="button"
                        variant="secondary"
