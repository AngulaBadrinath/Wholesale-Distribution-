...
import { Category, CategorySelectOption, CategoryStatusOption, CategoryStatus } from '@/types';
import {
    FolderTree,
    ArrowLeft,
    CheckCircle2,
...
export default function CategoryEdit({
    ...
    const { data, setData, put, processing, errors } = useForm<CategoryEditFormData>({
        name: category.name || '',
        code: category.code || '',
    ...
    return (
        <AppLayout title={`Edit Category: ${category.name}`}>
            <Head title={`Edit Category ${category.code}`} />
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <div>
                    <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1">
                        ...
                        <span>Master Classification / Edit: {category.code}</span>
                    </div>
                    <h1 className="text-2xl font-bold tracking-tight text-foreground">
                        Edit Category
                    </h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        Update category identity, hierarchical parent placement, sort order, and description.
                    </p>
                </div>

                {/* Status Notice Banner if has dependencies */}
                {(hasProducts || hasChildren) && (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3.5 flex items-start gap-3">
                        ...
                        <div className="text-xs text-amber-700 dark:text-amber-300">
                            <p className="font-semibold">Categorization Dependency Notice</p>
                            <p className="mt-0.5 text-muted-foreground">
                                This category currently has {category.products_count ?? 0} attached product(s) and{' '}
                                {category.children_count ?? 0} direct subcategory(ies). Reparenting will safely move this entire subtree.
                            </p>
                        </div>
                    </div>
                )}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-[11px] font-mono text-muted-foreground">Unique</span>
                                    </div>
                                    <Input
                                        id="code"
                                        type="text"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-[11px] text-muted-foreground">Cycle-Safe Filtered</span>
                                    </div>
                                    <select
                                        id="parent_id"
                                    ...
                                    <p className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1">
                                        ...
                                        Self and all current descendants are automatically excluded to prevent cycles.
                                    </p>
                                </div>
